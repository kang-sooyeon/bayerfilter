...
__kernel void red_blue_vertical_sum(__global float2 *I, __global float2 *P,
                                    __global float2 *sumI, __global float2 *sumP,
                                    __global float2 *sumII, __global float2 *sumIP,
                                    int H, int W) {
  ...
  II_sum5 = mad(I_v, I_v, mad(I_v2, I_v2, I_v4*I_v4));
  II_sum6 = mad(I_v1, I_v1, mad(I_v3, I_v3, I_v5*I_v5));
  IP_sum5 = mad(I_v, P_v, mad(I_v2, P_v2, I_v4*P_v4));
  IP_sum6 = mad(I_v1, P_v1, mad(I_v3, P_v3, I_v5*P_v5));
  ...
  for (int oh = 1; oh < 5; oh ++) {
    ...
    II_sum5 = mad(I_v4_, I_v4_, mad(I_v2_, I_v2_,
          mad(I_v, I_v, mad(I_v2, I_v2, I_v4*I_v4))));
    II_sum6 = mad(I_v5_, I_v5_, mad(I_v3_, I_v3_,
          mad(I_v1_, I_v1_, mad(I_v1, I_v1, mad(I_v3, I_v3, I_v5*I_v5)))));
    IP_sum5 = mad(I_v4_, P_v4_, mad(I_v2_, P_v2_,
          mad(I_v, P_v, mad(I_v2, P_v2, I_v4*P_v4))));
    IP_sum6 = mad(I_v5_, P_v5_, mad(I_v3_, P_v3_,
          mad(I_v1_, P_v1_, mad(I_v1, P_v1, mad(I_v3, P_v3, I_v5*P_v5)))));
  ...
  for( int oh = H-5; oh < H; oh ++ ) {
    ...
    II_sum5 = mad(I_v4_, I_v4_, mad(I_v2_, I_v2_,
          mad(I_v, I_v, mad(I_v2, I_v2, I_v4*I_v4))));
    II_sum6 = mad(I_v5_, I_v5_, mad(I_v3_, I_v3_,
          mad(I_v1_, I_v1_, mad(I_v1, I_v1, I_v3*I_v3))));
    IP_sum5 = mad(I_v4_, P_v4_, mad(I_v2_, P_v2_,
          mad(I_v, P_v, mad(I_v2, P_v2, I_v4*P_v4))));
    IP_sum6 = mad(I_v5_, P_v5_, mad(I_v3_, P_v3_,
          mad(I_v1_, P_v1_, mad(I_v1, P_v1, I_v3*P_v3))));
...
__kernel void red_blue_residual_interpolation_and_add_tentative(__global float *residualRB,
                                                                __global float *tentativeR,
                                                                __global float *tentativeB,
                                                                int H, int W) {
  int i = get_global_id(0);
  ...
  int iw, ih, flag;
  float value_red=0, value_blue=0;
...
__kernel void horizontal_sum_and_linear_coefficient(__global float4 *I, __global float4 *P,
                                                    __global float4 *II, __global float4 *IP,
                                                    __global float *red_coa,
                                                    __global float *red_cob,
                                                    __global float *blue_coa,
                                                    __global float *blue_cob,
                                                    int H, int W) {
  int i = get_global_id(0);
  if (i >= H * W) return;
  int oh = i / W;
  ...
  float4 pre1, pre2, post1, post2;

  float I_even_sum, I_odd_sum, P_even_sum, P_odd_sum;
  float II_even_sum, II_odd_sum, IP_even_sum, IP_odd_sum;

  bool remainder_is_even = ((remainder & 0x1) == 1) ? false : true;

  ///////////////////////////////////////////////////////////////// buffer I
  /////////////////////////////////////////// mid //
  float4 mid = I[quiotient];
  ...
  float red_coa_v = rem ? IP_odd_sum : IP_even_sum;
  red_coa[i] = red_coa_v;

  float red_cob_v = rem ? P_odd_sum - IP_odd_sum * I_odd_sum :
                          P_even_sum - IP_even_sum * I_even_sum;
  red_cob[i] = red_cob_v;

  float blue_coa_v = rem ? IP_even_sum : IP_odd_sum;
  blue_coa[i] = blue_coa_v;

  float blue_cob_v = rem ? P_even_sum - IP_even_sum * I_even_sum :
                           P_odd_sum - IP_odd_sum * I_odd_sum;
  blue_cob[i] = blue_cob_v;
}

__kernel void coefficient_vertical_sum(__global float2 *in, __global float2 *out, int H, int W) {
  ...
  float2 _v5, _v4, _v3, _v2, _v1, v, v1, v2, v3, v4, v5;
...
__kernel void horizontal_sum_and_tentative_residual(__global float *green,
                                                    __global float *raw,
                                                    __global float4 *redA,
                                                    __global float4 *redB,
                                                    __global float4 *blueA,
                                                    __global float4 *blueB,
                                                    __global float *tentativeR,
                                                    __global float *tentativeB,
                                                    __global float *residualRB,
                                                    int H, int W) {
  int i = get_global_id(0);
  if (i >= H * W) return;
  int oh = i / W;
  ...
  //////////////////////////////////////////////////////////////// redA buffer
  float4 mid = redA[quiotient];
  float A_sum = mid.x + mid.y + mid.z + mid.w;
  ...
  //////////////////////////////////////////////////////////////// redB buffer
  mid = redB[quiotient];
  float B_sum = mid.x + mid.y + mid.z + mid.w;
  ...
  //////////////////////////////////////////////////////////////// tentative red
  float value_green = green[i];
  float tentaR = (A_sum/divisor_h) * value_green + (B_sum/divisor_h);
  tentativeR[i] = tentaR;
  ...
  //////////////////////////////////////////////////////////////// tentative blue
  float tentaB = (A_sum/divisor_h) * value_green + (B_sum/divisor_h);
  tentativeB[i] = tentaB; 
